...
def get_images(link, cookie, ua, retries=2, delay=5):
    ...
    def extract_images(r_text):
        ...
        if img_urls:
            print(f"[DEBUG] ✅ {len(img_urls)} images détectées via balises <img>.")
        return img_urls

    # --- Phase 1 : tentative directe sans FlareSolverr
    try:
        r = make_request(link, cookie, ua)
        print(f"[DEBUG] 📡 Requête HTTP directe reçue (len={len(r.text)})")
...
class MangaApp:
    ...
    def setup_ui(self):
        ...
        entry = tk.Entry(filter_group, textvariable=self.filter_text, width=25)
        entry.pack(side="left", padx=5)
        entry.bind("<KeyRelease>", lambda e: self.apply_filter())
        tk.Button(filter_group, text="❌", command=self.clear_filter).pack(side="left")
        ...
        self.canvas = tk.Canvas(canvas_frame, height=300, bg="white", highlightthickness=0)
        self.scrollbar = ttk.Scrollbar(canvas_frame, orient="vertical", command=self.canvas.yview)
        self.canvas.pack(side="left", fill="both", expand=True)
        self.scrollbar.pack(side="right", fill="y")
        self.canvas.configure(yscrollcommand=self.scrollbar.set)

        self.vol_frame = tk.Frame(self.canvas, bg=self.root["bg"])
