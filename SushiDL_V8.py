    ...
    def extract_images(r_text):
        ...
        if json_str:
            try:
                ...
                if images:
                    print(f"[INFO] ✅ {len(images)} images détectées via JSON.")
                    return images
            except Exception as e:
        ...
        if img_urls:
            print(f"[INFO] ✅ {len(img_urls)} images détectées via balises <img>.")
        return img_urls
...
class MangaApp:
    ...
    def fetch_manga_image(self, url):
        """Récupère et affiche l'image du manga à partir de l'URL de la page"""
        try:
            # Récupérer le cookie et l'User-Agent depuis l'interface
            cookie = self.get_cookie(url)  # cf_clearance cookie
            user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.0.0 Safari/537.36"

            # Si l'URL vient de sushiscan.fr, utiliser FlareSolverr
            if "sushiscan.fr" in url:
                self.log("URL détectée sur sushiscan.fr, récupération via FlareSolverr...", level="warning")

                # Utilisation de FlareSolverr pour contourner Cloudflare
                flaresolverr_response = fetch_with_flaresolverr(url, MangaApp.flaresolverr_url_static)

                if not flaresolverr_response:
                    self.log("Échec de FlareSolverr, aucune page récupérée.", level="error")
                    return

                # Ré-analyser la page obtenue via FlareSolverr
                soup = BeautifulSoup(flaresolverr_response, 'html.parser')
                thumb_container = soup.find("div", class_="thumb-container")

                if not thumb_container:
                    self.log("Conteneur de la couverture introuvable", level="error")
                    return

                # 2. Cibler l'image dans ce conteneur
                img_tag = thumb_container.find("img", {"src": True})

                if not img_tag or not img_tag.get("src"):
                    self.log("Balise image manquante dans le conteneur", level="error")
                    return

                # 3. Nettoyer l'URL (enlever les paramètres comme '?resize=...')
                raw_url = img_tag["src"].split("?")[0]
                img_url = urllib.parse.urljoin(url, raw_url)

                # 4. Vérifications finales
                if img_url.startswith("data:image"):
                    self.log("Image encodée en base64 (invalide)", level="error")
                    return
                if not any(img_url.lower().endswith(ext) for ext in [".jpg", ".jpeg", ".png", ".webp"]):
                    self.log(f"Format d'image non supporté : {img_url}", level="warning")
                    return

                self.log(f"Nombre d'images dans la page : {len(soup.find_all('img'))}", level="info")
                img_tag = None
                for img in soup.find_all("img", src=True):
                    src = img["src"]
                    if not src.startswith("data:") and any(src.lower().endswith(ext) for ext in [".jpg", ".jpeg", ".png", ".webp"]):
                        img_tag = img
                        break
                if img_tag and any(img_tag['src'].endswith(ext) for ext in [".jpg", ".jpeg", ".png", ".webp"]):
                    img_url = urllib.parse.urljoin(url, img_tag['src'])

                    self.log(f"URL de l'image trouvée via FlareSolverr : {img_url}")

                    # Vérifier que l'URL n'est pas en base64
                    if img_url.startswith("data:image"):
                        self.log("L'URL de l'image est en base64, ce n'est pas une image valide.", level="error")
                        return

                    # Télécharger l'image avec les en-têtes appropriés
                    image_headers = {
                    ...
                    self.log("Image affichée avec succès via FlareSolverr.", level="success")
                else:
                    self.log("Image non trouvée via FlareSolverr.", level="error")

            else:
                # Si l'URL n'est pas de sushiscan.fr, tenter une récupération directe
                headers = {
                    'User-Agent': user_agent,
                    'Cookie': f'cf_clearance={cookie}',  # Ajouter le cookie cf_clearance
                }

                # Envoyer la requête pour récupérer la page du manga
                response = requests.get(url, headers=headers)
                response.raise_for_status()  # Vérifie si la requête a réussi

                # Analyser la page pour récupérer l'URL de l'image
                soup = BeautifulSoup(response.text, 'html.parser')
                img_tag = soup.find('img', class_='attachment- size- wp-post-image')

                if img_tag:
                    img_url = img_tag['src']  # Récupérer l'URL de l'image depuis l'attribut src
                    img_url = urllib.parse.urljoin(url, img_url)  # Compléter l'URL si nécessaire

                    self.log(f"URL de l'image trouvée : {img_url}")
                    ...
                    # Récupérer l'image avec les en-têtes appropriés
                    image_headers = {
                        'User-Agent': user_agent,
                        'Cookie': f'cf_clearance={cookie}',
                        'Referer': url,
                    ...
                    self.manga_image_label = tk.Label(self.root, image=img_tk)
                    self.manga_image_label.image = img_tk  # Conserver une référence à l'image
                    self.manga_image_label.place(x=10, y=10)
                    ...
                    self.log("Image affichée avec succès.", level="success")
                else:
                    self.log("Image non trouvée sur la page", level="error")

        except requests.exceptions.RequestException as e:
            self.log(f"Erreur lors de la récupération de l'image : {e}", level="error")
    ...
    def setup_ui(self):
        ...
        # === Champ URL du manga + bouton Analyser
        ...
        tk.Button(self.root, text="Analyser", command=self.load_volumes, width=30).pack(pady=(5, 10))
