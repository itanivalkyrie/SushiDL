    ...
    def download_selected(self):
        ...
        def task():
            ...
            def per_image_progress(done, total_images):
                ...
                percent = round((done / total_images) * 100, 1)
                timestamp = time.strftime("%H:%M:%S")
                ...
                if self.image_progress_index is None:
                    ...
                    self.log_text.delete(self.image_progress_index, str(float(self.image_progress_index) + 1))
                    self.log_text.insert(self.image_progress_index, line + "\n", "info")
                self.log_text.configure(state="disabled")
                self.log_text.see("end")
                # 🔁 Mise à jour de la barre de progression visuelle
                percent = (done / total_images) * 100
            ...
            for idx, (vol, link) in enumerate(selected):
                ...
                self.image_progress_index = None  # reset pour chaque volume
                self.progress.set(0)
                ...
                if images:
                    ...
                    # 🔁 Mise à jour finale de la progression (optionnelle)
