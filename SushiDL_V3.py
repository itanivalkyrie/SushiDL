...
def download_volume(volume, images, title, cookie, ua, logger, cancel_event):
    if cancel_event.is_set():
        return
    clean_title = sanitize_folder_name(title)
    clean_volume = sanitize_folder_name(volume)
...
def save_cookie_cache(cf_clearance, ua):
    data = {
...
def load_cookie_cache():
    ...
    try:
        ...
        if age <= 86400:
            return data.get("cf_clearance"), data.get("ua")
    except Exception as e:
...
class MangaApp:
        ...
        def task():
            ...
            for idx, (vol, link) in enumerate(selected):
                ...
                if images:
                    ...
                    download_volume(vol, images, self.title, self.cookie.get().strip(), self.ua.get().strip(), self.log, self.cancel_event)
                else:
    ...
    def save_current_cookie(self):
        save_cookie_cache(self.cookie.get().strip(), self.ua.get().strip())
